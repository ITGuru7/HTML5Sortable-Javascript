		...
		<div id="deleteModal" class="modal fade">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					...
					<!-- Modal body -->
					<div class="modal-body">
						<h4 class="delete-title">Are you sure you want to delete this?</h4>
					</div>

					<!-- Modal footer -->
					<div class="modal-footer">
						<button class="btn text-white font-weight-bold" id="btn-delete-yes" data-dismiss="modal">Yes</button>
						<button class="btn text-white font-weight-bold" id="btn-delete-no" data-dismiss="modal" autofocus>No</button>
					</div>
				</div>
			</div>
		</div>
	...
	<!-- Temp Data -->
		<div>
			<div id="temp-parent" class="row my-4 p-0 pt-1 nested-list d-none">
				<div class="row m-1 p-2 border-radius-3 bg-transparent w-100 d-flex justify-content-between">
					...
					<div class="col p-0 d-flex justify-content-end">
						...
						<button class="btn btn-delete-parent py-1 text-white font-weight-bold" data-toggle="modal" data-target="#deleteModal"><i class="fa fa-trash"></i>&nbsp;<span class="d-none d-md-block float-right text-white">Delete</span></button>
				...
				<div class="container-fluid p-0 pb-3 nested-children-container">
				</div>
			</div>
			...
			<div id="temp-child" class="row mx-1 my-2 p-2 border-radius-3 nested-child bg-white d-none">
				...
				<div class="col p-0 d-flex justify-content-end">
					<button class="btn btn-edit py-1 text-white font-weight-bold" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i>&nbsp;<span class="d-none d-md-block float-right text-white">Edit</span></button>
					<button class="btn btn-delete-child py-1 text-white font-weight-bold" data-toggle="modal" data-target="#deleteModal"><i class="fa fa-trash"></i>&nbsp;<span class="d-none d-md-block float-right text-white">Delete</span></button>
				</div>
			</div>
...
//				event.preventDefault();
		...
		function onEditContent() {
			...
			$('#editModal').find('.edit-title').text(jsonData[index]['data-label']);
			$('#editModal').find('textarea').val(jsonData[index]['data-content']);
		}

		function onSaveContent() {
			var data_id = $('#editModal').find('input[type=hidden]').val();
			var data_content = $('#editModal').find('textarea').val();
	...
	//	delete parent/child
		function onDeleteParent() {
			...
			$('#deleteModal').find('input[type=hidden]').val(data_id);
			$('button#btn-delete-yes').off('click');
			$('button#btn-delete-yes').on('click', onDeleteParentYes);
		}
		function onDeleteParentYes() {
			$('#deleteModal').modal('hide');
			var data_id = $('#deleteModal').find('input[type=hidden]').val();
			$('.nested-list[data-id="' + data_id + '"]').remove();
			reload_sortable();
		...
		function onDeleteChild() {
			...
			$('#deleteModal').find('input[type=hidden]').val(data_id);
			$('button#btn-delete-yes').off('click');
			$('button#btn-delete-yes').on('click', onDeleteChildYes);
		}
		function onDeleteChildYes() {
			$('#deleteModal').modal('hide');
			var data_id = $('#deleteModal').find('input[type=hidden]').val();
			$('.nested-child[data-id="' + data_id + '"]').remove();
			reload_sortable();
